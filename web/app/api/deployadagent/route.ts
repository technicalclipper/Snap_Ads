...
export async function POST(req: NextRequest) {
  ...
  const { advertiser, name, description, ipfsVideoLink, totalFunded } = body;

  const deployadTool = createTool({
    id: "deployad-tool",
    description:
      "Deploy an advertisement by sending funds to the platform’s ad spot. The ad details, including advertiser’s address, ad name, description, IPFS video link, and total funds, are provided. Deploy the ad to an available spot on the platform.",
    schema: z.object({
      ...
      totalFunded: z
        .string()
        .describe("Total amount of funds provided for the ad"),
    }),
    //@ts-expect-error
    execute: async ({
      advertiser,
  ...
  state.messages.push(
    user(
      `advertiser address: ${advertiser}, ad name: '${name}', description: '${description}', IPFS video link: '${ipfsVideoLink}', total funds: '${totalFunded}'`
      //message!
    )
  );
